...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    res.json({ token, user: { id, username, email, display_name: display_name || username, avatar: '' } });
...
app.get('/api/auth/me', authenticateToken, (req, res) => {
  const user = db.prepare('SELECT id, username, email, display_name, avatar, bio, location, website FROM users WHERE id = ?').get(req.user.id);
  res.json(user);
});
...
app.get('/api/users/:id', authenticateToken, (req, res) => res.json(db.prepare('SELECT id, username, display_name, avatar, bio, location, website FROM users WHERE id = ?').get(req.params.id)));

app.get('/api/users/:id/posts', authenticateToken, (req, res) => {
  res.json(db.prepare('SELECT p.*, u.username, u.display_name, u.avatar FROM posts p JOIN users u ON p.user_id = u.id WHERE p.user_id = ? ORDER BY p.created_at DESC').all(req.params.id));
});
