        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            {isOwnProfile && !editing && (
              <button onClick={() => setEditing(true)} className="mt-4 px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition text-sm">
                Edit Profile
              </button>
            )}
          </div>
        ...
        {editing && (
          <form onSubmit={handleEditSubmit} className="mt-6 pt-6 border-t border-dark-100">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Display Name</label>
                <input type="text" value={editForm.display_name || ''} onChange={e => setEditForm({ ...editForm, display_name: e.target.value })} className="w-full px-4 py-2 bg-dark-100 border border-dark-100 rounded-lg text-white" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Location</label>
                <input type="text" value={editForm.location || ''} onChange={e => setEditForm({ ...editForm, location: e.target.value })} className="w-full px-4 py-2 bg-dark-100 border border-dark-100 rounded-lg text-white" />
              </div>
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-300 mb-1">Bio</label>
                <textarea value={editForm.bio || ''} onChange={e => setEditForm({ ...editForm, bio: e.target.value })} className="w-full px-4 py-2 bg-dark-100 border border-dark-100 rounded-lg text-white" rows={3} />
              </div>
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-300 mb-1">Profile Picture</label>
                <input type="file" accept="image/*" onChange={e => setEditAvatar(e.target.files[0])} className="text-sm text-gray-400" />
              </div>
            </div>
            <div className="flex gap-2 mt-6">
              <button type="submit" className="px-6 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition">Save Changes</button>
              <button type="button" onClick={() => setEditing(false)} className="px-6 py-2 bg-dark-100 text-gray-300 rounded-lg">Cancel</button>
            </div>
          </form>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {members.map(member => (
          <Link key={member.id} to={`/profile/${member.id}`}>
            <div className="bg-dark-200 rounded-xl shadow-lg border border-dark-100 p-4 hover:border-primary-600 transition">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-full bg-primary-900 flex items-center justify-center">
                  {member.avatar ? <img src={getImageUrl(member.avatar)} className="w-full h-full rounded-full object-cover" /> : <span className="text-primary-400 font-bold">{member.username[0].toUpperCase()}</span>}
                </div>
                <div>
